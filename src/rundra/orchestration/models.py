...
class ExecutionPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if self.version in {4, 5, 6, 7, 8, 9}:
            ...
            if self.version in {5, 6, 7, 8, 9}:
                ...
                if self.version >= 6 and self.strategy == WORKER_POOL:
                    ...
                    for value in (
                        ...
                        self.requested_task_slots_per_worker,
                    )
                ):
                    raise ValueError(
                        "multi-array plans cannot contain requested worker scale"
                    )
                elif any(
                    value is not None
                    for value in (
                    ...
                    raise ValueError("ExecutionPlan v5 cannot contain v6 scale fields")
            elif any(
                value is not None
                for value in (
