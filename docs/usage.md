...
| `SCHEDULER_QUERY_FAILED` or `ACCOUNTING_PENDING` | Retry `status`; verify the configured scheduler's query commands. Dependency-pending workers do not require journals yet. Rundra accepts identical events visible in overlapping atomic fragments but rejects contradictory Task outcomes. For Slurm, `sacct` is optional and `squeue`/`scontrol` provide fallback paths; for OpenPBS, verify `qstat`. |
