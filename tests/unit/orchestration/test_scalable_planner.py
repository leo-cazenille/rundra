...
def test_v5_worker_plan_clamps_slots_for_an_underfilled_worker() -> None:
    policy = ExecutionPolicy(
        100_000_000,
        1,
        320,
    ...
    assert plan.task_slots_per_worker == 4
    assert plan.concurrent_task_capacity == 4
    assert plan.max_lane_depth == 1
    assert plan.worker_resources == ResourceRequest(
        nodes=1,
        tasks=4,
        cpus_per_task=1,
